...
export default function NumpadSVG({ keymap, selectedKey, onKeyClick }) {
  return (
    <svg viewBox="0 0 670 560" className="w-full h-full max-h-[55vh] drop-shadow-2xl" onClick={() => onKeyClick(null)}>
      ...
      {numpadKeys.map(k => <KeyNode key={k.id} k={k} isSel={selectedKey === k.id} mappedCode={keymap[k.id]} onKeyClick={onKeyClick} />)}
    </svg>
  );
}
